...
# Load dataset
infiltration = pd.read_csv("I:\\My Drive\\Colab Notebooks\\datasets\\data\\Prepared\\infiltration_after_feature_selection.csv")
print(infiltration.head())

infiltration = shuffle(infiltration).reset_index(drop=True)
# Split dataset
X = infiltration.drop(columns=["Timestamp", 'Label'])
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        self.model.fit(self.X_train, self.y_train, validation_data=(self.X_val, self.y_val), epochs=5, batch_size=512, verbose=VERBOSE)
